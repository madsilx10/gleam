...
def queue_contestant(session, csrf, email, name, campaign_key):
    ...
    task_signature = r.headers.get("task-signature", "")
    print(f"Queue contestant: {r.status_code}")
    print(f"task-signature: {'OK' if task_signature else 'not found'}")
    return r.status_code in [200, 201, 202], new_csrf, task_signature

def claim_contestant(session, csrf, campaign_key, task_signature=None, x_ref=None):
    """Step 2: Pilih email sebagai metode login"""
    headers = make_headers(csrf, campaign_key, x_ref)
    if task_signature:
        headers["task-signature"] = task_signature
    r = session.patch(
        "https://gleam.io/claim-contestant",
        headers={**headers, "Content-Length": "0"},
    )
    new_csrf = session.cookies.get("XSRF-TOKEN", csrf)
    print(f"Claim contestant: {r.status_code}")
...
def send_recovery_code(session, csrf, email, campaign_key, x_ref=None):
    ...
    new_csrf = session.cookies.get("XSRF-TOKEN", csrf)
    print(f"Send recovery code: {r.status_code}")
    return r.status_code in [200, 201, 202], new_csrf
...
def recover_contestant(session, csrf, email, code, campaign_key, x_ref=None):
    ...
    print(f"Recover contestant: {r.status_code}")
...
def main():
    ...
    while True:
        ...
        ok, csrf, task_sig = queue_contestant(session, csrf, email, name, campaign_key)
        if not ok:
        ...
        print("⏳ Pilih email...")
        ok, csrf = claim_contestant(session, csrf, campaign_key, task_sig, x_ref)
        if not ok:
