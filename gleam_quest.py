...
def get_entry_methods(campaign_key, cookies, xsrf, x_ref):
    """Fetch semua entry methods (quest) dari halaman campaign"""
    cookie_str = "; ".join([f"{k}={v}" for k, v in cookies.items()])
    r = requests.get(
        f"https://gleam.io/{campaign_key}.json",
        headers={
            "User-Agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Safari/537.36",
    ...
    if r.status_code == 200:
        data = r.json()
        return data.get("campaign", {}).get("entry_methods", [])
    print(f"Get entry methods: {r.status_code}")
    return []
