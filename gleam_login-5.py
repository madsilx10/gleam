...
def queue_contestant(session, csrf, email, name, campaign_key):
    ...
    print(f"Queue contestant: {r.status_code} | {r.text[:100]}")
    # Ambil x_ref dari response kalau ada
    x_ref = None
    try:
        data = r.json()
        x_ref = data.get("contestant", {}).get("token") or data.get("token")
    except:
        pass
...
def main():
    ...
    while True:
        ...
        ok, csrf, x_ref = queue_contestant(session, csrf, email, name, campaign_key)
        if not ok:
